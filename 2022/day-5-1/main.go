...
func solveProblem(fileName string, indexToStackMap map[int][]string) string {
	...
	for fileScanner.Scan() {
		...
		for i := 1 ; i <= numberOfCratesToMove ; i++ {
			stackToMoveTo = append(stackToMoveTo, stackToMoveFrom[len(stackToMoveFrom)-1])
			stackToMoveFrom = stackToMoveFrom[:len(stackToMoveFrom)-1]
		}
