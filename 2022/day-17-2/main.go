...
func main() {
	fmt.Println(solveProblem("input.txt",1000000000000))
}
...
func solveProblem(fileName string, target int) int {
	...
	for ; rested_rocks < target ; {
		...
		// state
		if (rested_rocks > 0 && trackState){
			heightsPerRound = append(heightsPerRound, highestRock)

			state := ""
			state += fmt.Sprintf("%v-%v",sequence_index % len (sequence),shape_type_number%5)
			for x:= 0 ; x < 7 ; x++ {
				found := false
				for y:= highestRock ; y >= 0 ; y-- {
					if (cave[y][x]) == "#" {
						state += fmt.Sprintf("-%v",abs(highestRock-y))
						found = true
						break
					}
				}
				if !found {
					state += fmt.Sprintf("-%v",abs(highestRock))
				}
			}

			fmt.Println(rested_rocks, " ," , state, ",", highestRock+1)
			
			//have we seen this before
			for i := 0 ; i < len(overallState) ; i++ {
				if overallState[i] == state {
					fmt.Println("cycle found here")
				}
			}

			overallState = append(overallState, state)
		}
