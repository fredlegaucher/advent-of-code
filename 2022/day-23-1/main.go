...
func solveProblem(fileName string) int {
	mapOfElves := setupProblem(fileName)
        ...
        smallestColumn,smallestRow := math.MaxInt,math.MaxInt 
        highestRow,highestColumn := math.MinInt,math.MinInt  
        for _, e := range mapOfElves { 
                if e.column < smallestColumn { 
                        smallestColumn = e.column 
                } 
                if e.row < smallestRow { 
                        smallestRow = e.row 
                } 
                if e.column > highestColumn { 
                        highestColumn = e.column 
                } 
                if e.row > highestRow { 
                        highestRow = e.row 
                } 
        ...
        return ((highestRow-smallestRow+1)*(highestColumn-smallestColumn+1))-len(mapOfElves)
} 
